...
export default function SessionDrawer() {
  ...
  const { concepts, reset, topic, setTopic, sessions, loadSession } = useFeynmanStore();
              ...
              <div className="space-y-6 flex-1 overflow-y-auto">
                <div>
                   ...
                   <div className="space-y-2">
                     {sessions.length === 0 && (
                       <div className="text-white/20 text-sm italic p-2">No history yet</div>
                     )}
                     {sessions.map((session) => (
                       <button
                         key={session.id}
                         onClick={() => loadSession(session.id)}
                         className="w-full text-left p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors border border-white/5 group"
                       >
                         <div className="font-medium text-white text-sm group-hover:text-blue-300 transition-colors truncate">
                           {session.topic}
                         </div>
                         <div className="text-xs text-white/40 mt-1 truncate">
                           {new Date(session.date).toLocaleDateString()} • {session.preview}
                         </div>
                       </button>
                     ))}
                   </div>
